  ...
  implements Listener
{
  ...
  {
    if (!event.getPlayer().hasPermission("ipmatcher.authentication")) {
      return;
    }
    if (Core.getInstance().getDB().getIp(event.getPlayer()) == null)
    {
      Core.getInstance().getDB().setIp(event.getPlayer(), 
        event.getPlayer().getAddress().getAddress().getHostAddress());
      event.getPlayer().sendMessage(
        "§aLooks like it's your first time joining as staff, you have been added to the authentication system.");
      return;
    }
    if (Core.getInstance().getDB().getResetQueue().contains(event.getPlayer().getUniqueId().toString()))
    {
      event.getPlayer().sendMessage(
        "§aLooks like an admin reset your IP. Welcome, §e" + event.getPlayer().getName() + "§a.");
      Core.getInstance().getDB().setIp(event.getPlayer(), 
        event.getPlayer().getAddress().getAddress().getHostAddress());
    ...
    if (!Core.getInstance().getDB().getIp(event.getPlayer()).equals(event.getPlayer().getAddress().getAddress().getHostAddress()))
    {
      event.getPlayer().kickPlayer(
        "§cAuthentication failed! Looks like your IP has changed, please contact an admin.");
      return;
    }
